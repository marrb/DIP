...
class STAM(nn.Module):
    ...
    - Helps improve temporal consistency by reducting covariate shift
    - Uses instance centering to normalize feature distributions
    """
    
    """
    Init method for STAM
    """
    ...
    """
    Perform forward pass of STAM
    """
    def forward(self, x):
        #Perform instance centering
        x_centered = x - x.mean(dim = (1, 2, 3), keepdim = True)
        return self.norm(x_centered)
...
class UNet3DConditionModelEI2(UNet3DConditionModel):
    ...
    def __init__(self, **kwargs):
        ...
        #Init STAM and FFAM modules
        self.stam = STAM(dim=self.in_channels)
        self.ffam = nn.ModuleList([
            FFAM(dim=block_out_channel) for block_out_channel in self.out_channels
        ])
        
    def forward(self, sample, timestep, encoder_hidden_states, class_labels=None, attention_mask=None, return_dict=True): 
        """
        Perform forward pass of UNet3DConditionModelEI2
        - Applies STAM before feature extraction
        - Passes trough UNet3D model
        - Applies FFAM after each stage to refine output
        """
